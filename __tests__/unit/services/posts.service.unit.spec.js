...
describe('Posts Servcie Unit Test', () => {
  ...
  test('updatePost Method', async () => {
    const samplePosts = {
      ...
      "imgUrl": "35f3c58c10fe7974e0148d01155d988d1d037afec1c74a346bdc2b4ea076f54e",
      "likeCount": 0,
      "commentCount": 0,
      "star": 1,
    ...
    //Received
    const updatePost = await postsService.updatePost(1, 1, "test address", "test Post Content 1 test Post Content 1", 1, "testStoreName1", "testPlaceInfoId1", "37.1111", "127.1111", "categoryName1");

    expect(mockPostRepository.findPostByPostId).toHaveBeenCalledTimes(1);
    expect(mockPostRepository.findPostByPostId).toHaveBeenCalledWith(samplePosts.postId);

    expect(mockPostRepository.updatePost).toHaveBeenCalledTimes(1);

    //Expected
    expect(mockPostRepository.updatePost).toHaveBeenCalledWith(samplePosts.postId, samplePosts.UserId, samplePosts.address, samplePosts.content, samplePosts.star, samplePosts.storeName, samplePosts.placeInfoId, samplePosts.latitude, samplePosts.longitude, samplePosts.categoryName);
  ...
  test('updatePost Method By 수정할 권한이 존재하지 않습니다.', async () => {
    ...
    const samplePosts = {
      ...
      "LocationId": 1,
      "content": "test Post Content 1 test Post Content 1",
      "imgUrl": "eb4e64bdd9cff142c02bd567ec915a5de1f304c20e348fb25d0bf6aa67da9bf5,eb4e64bdd9cff142c02bd567ec915a5de1f304c20e348fb25d0bf6aa67da9bf5",
      "likeCount": 1,
      "commentCount": 1,
      "star": 1,
    ...
    try {
      await postsService.updatePost(12, 11, "서울시 노원구 테스트 1동", "test Post Content 1 test Post Content 1", 1, "testStoreName", "testPlaceInfoId1", "37.1111", "127.1111", "categoryName1")
  ...
  test('findPostById Method', async () => {
    const samplePosts = {
      ...
      "likeCount": 0,
      "commentCount": 1,
      "imgUrl": "818a74719a9a4658413e7fcec94c59a453b29627a5e51c7ecd079fa61d50701e",
      "star": 1,
      "User": {
        "userId": 1,
        "nickname": "testNickname",
        "imgUrl": "0b2f746651bb7e903bd2c985714170b8746f0fc6d9a966ba31476e515495ebd1"
      },
      "Location": {
        "locationId": 1,
        "address": "서울 노원구 테스트 1동",
  ...
  test('deletePost Method', async () => {
    ...
    expect(mockPostRepository.deletePost).toHaveBeenCalledWith(samplePosts.UserId, samplePosts.postId);
