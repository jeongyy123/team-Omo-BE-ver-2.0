export class MainController {
  ...
  getPoplurPosts = async (req, res, next) => {
    try {
      ...
      const popularPosts = await this.mainService.getPoplurPosts(
        districtName,
        limit,
      );
  ...
  getRecentPosts = async (req, res, next) => {
    try {
      ...
      const recentPosts = await this.mainService.getRecentPosts(
        districtName,
        limit,
        categoryName,
      );
  ...
  getRecentComments = async (req, res, next) => {
    try {
      ...
      const recentComments = await this.mainService.getRecentComments(
        districtName,
        limit,
      );
