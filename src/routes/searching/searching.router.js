...
import { getManyImagesS3, getImageS3, getProfileImageS3 } from '../../utils/getImageS3.js'
import {
  S3Client,
  PutObjectCommand,
  GetObjectCommand,
...
router.get("/posts/main/searching", async (req, res, next) => {
  try {
    ...
    const { nickname, storeName } = req.body;
    ...
    if (nickname && storeName) {
      return res
        .status(400)
        .json({ message: "nickname 또는 storeName 둘 중 하나만 입력해주세요." });
    }
    ...
    const imgUrlsArray = resultData.map((arr) => arr.Posts[0].imgUrl.split(','));
