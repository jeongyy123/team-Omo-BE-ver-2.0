...
export class BookmarkService {
  ...
  createBookmark = async (locationId, userId) => {
    const findLocationByLocationId = await this.bookmarkRepository.findLocationByLocationId(locationId);
    ...
    const findBookmarkByLocationIdAndUserId = await this.bookmarkRepository.findBookmarkByLocationIdAndUserId(locationId, userId);
  ...
  findLocationByLocationId = async (locationId) => {
    const location = await this.bookmarkRepository.findLocationByLocationId(locationId);

    return location;
  }

  findBookmarkByLocationIdAndUserId = async (locationId, userId) => {
    const bookmark = await this.bookmarkRepository.findBookmarkByLocationIdAndUserId(locationId, userId);

    return bookmark
  }

  getUserMapBookmarks = async (userId) => {
    const userBookmark = await this.bookmarkRepository.getUserMapBookmarks(userId);
